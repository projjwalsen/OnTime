...
async function runTests() {
  ...
  // 1a. List with search
  const catSearchRes = await makeRequest(
    'GET',
    `/api/v1/categories?search=${encodeURIComponent('spices')}`,
    undefined,
    superToken,
  );
  assert(
    catSearchRes.status === 200,
    `Category search failed: ${JSON.stringify(catSearchRes.body)}`,
  );
  assert(Array.isArray(catSearchRes.body.data.categories), 'categories should be an array');
  assert(
    catSearchRes.body.data.categories.some((c: any) => c.id === cat1.id),
    'Category 1 should be found in search',
  );
  assert(
    !catSearchRes.body.data.categories.some((c: any) => c.id === cat2.id),
    'Category 2 should not match "spices"',
  );

  // 1b. List with pagination
  const catPageRes = await makeRequest(
    'GET',
    `/api/v1/categories?search=${encodeURIComponent(catPrefix)}&page=1&limit=2`,
    undefined,
    superToken,
  );
  assert(
    catPageRes.status === 200,
    `Category pagination failed: ${JSON.stringify(catPageRes.body)}`,
  );
  assert(
    catPageRes.body.data.categories.length === 2,
    `Expected 2 categories, got ${catPageRes.body.data.categories.length}`,
  );
  assert(
    catPageRes.body.data.pagination.total === 3,
    `Expected total 3, got ${catPageRes.body.data.pagination.total}`,
  );
  ...
  // 2a. Search by SKU
  const prodSkuRes = await makeRequest(
    'GET',
    `/api/v1/products?search=${encodeURIComponent(`${prodSkuPrefix}-SAF`)}`,
    undefined,
    superToken,
  );
  assert(
    prodSkuRes.status === 200,
    `Product SKU search failed: ${JSON.stringify(prodSkuRes.body)}`,
  );
  ...
  const prodCatRes = await makeRequest(
    'GET',
    `/api/v1/products?categoryId=${cat1.id}`,
    undefined,
    superToken,
  );
  assert(
    prodCatRes.status === 200,
    `Product category filter failed: ${JSON.stringify(prodCatRes.body)}`,
  );
  assert(
    prodCatRes.body.data.products.every((p: any) => p.categoryId === cat1.id),
    'All products must belong to cat1',
  );

  // 2c. Filter by isActive
  const prodActiveRes = await makeRequest(
    'GET',
    `/api/v1/products?search=${encodeURIComponent(prodSkuPrefix)}&isActive=true`,
    undefined,
    superToken,
  );
  assert(
    prodActiveRes.status === 200,
    `Product active filter failed: ${JSON.stringify(prodActiveRes.body)}`,
  );
  assert(
    prodActiveRes.body.data.products.every((p: any) => p.isActive === true),
  ...
  const prodPriceRes = await makeRequest(
    'GET',
    `/api/v1/products?search=${encodeURIComponent(prodSkuPrefix)}&minPrice=100&maxPrice=300`,
    undefined,
    superToken,
  );
  assert(
    prodPriceRes.status === 200,
    `Product price range filter failed: ${JSON.stringify(prodPriceRes.body)}`,
  );
  assert(
    prodPriceRes.body.data.products.length === 1,
    'Expected exactly 1 product (Black Pepper 120)',
  );
  assert(prodPriceRes.body.data.products[0].id === prod2.id, 'Expected product 2');

  // 2e. Pagination
  const prodPageRes = await makeRequest(
    'GET',
    `/api/v1/products?search=${encodeURIComponent(prodSkuPrefix)}&page=1&limit=2`,
    undefined,
    superToken,
  );
  assert(
    prodPageRes.status === 200,
    `Product pagination failed: ${JSON.stringify(prodPageRes.body)}`,
  );
  ...
  console.log(
    '  ✔ Products search, filters (category, active, price range) and pagination validated',
  );
  ...
  const orgSearchRes = await makeRequest(
    'GET',
    `/api/v1/organisations?search=${encodeURIComponent(`Apex Retailers ${timestamp}`)}`,
    undefined,
    superToken,
  );
  ...
  const orgStatusRes = await makeRequest(
    'GET',
    `/api/v1/organisations?search=${encodeURIComponent(timestamp.toString())}&status=SUSPENDED`,
    undefined,
    superToken,
  );
  assert(
    orgStatusRes.status === 200,
    `Org status filter failed: ${JSON.stringify(orgStatusRes.body)}`,
  );
  ...
  const orgCityRes = await makeRequest(
    'GET',
    `/api/v1/organisations?search=${encodeURIComponent(timestamp.toString())}&city=Delhi`,
    undefined,
    superToken,
  );
  ...
  // 3d. Pagination
  const orgPageRes = await makeRequest(
    'GET',
    `/api/v1/organisations?search=${encodeURIComponent(timestamp.toString())}&page=1&limit=1`,
    undefined,
    superToken,
  );
  ...
  const userSearchRes = await makeRequest(
    'GET',
    `/api/v1/users?search=${encodeURIComponent(`Apex Staff Member ${timestamp}`)}`,
    undefined,
    superToken,
  );
  ...
  const userRoleRes = await makeRequest(
    'GET',
    `/api/v1/users?organisationId=${orgA.id}&role=STAFF`,
    undefined,
    superToken,
  );
  assert(
    userRoleRes.status === 200,
    `User role filter failed: ${JSON.stringify(userRoleRes.body)}`,
  );
  ...
  const userOrgRes = await makeRequest(
    'GET',
    `/api/v1/users?organisationId=${orgA.id}`,
    undefined,
    superToken,
  );
  ...
  // 4d. Pagination
  const userPageRes = await makeRequest(
    'GET',
    `/api/v1/users?organisationId=${orgA.id}&page=1&limit=1`,
    undefined,
    superToken,
  );
